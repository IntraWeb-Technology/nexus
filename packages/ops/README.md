...
| `vercel:list-env-keys` | `src/vercel-kv-list.ts` | Loads the module that exports `PORTAL_ENV_KEYS` (same as importing it from TypeScript; useful for smoke-checking resolution). |
...
These mirror the former `apps/iw-portal/scripts/lib` sources so ops scripts typecheck with `rootDir: src` and run under `tsx` with NodeNext-style `.js` import specifiers:
...
- `src/vercel-kv-list.ts` — `PORTAL_ENV_KEYS` (canonical; import from here or run `vercel:list-env-keys`)
