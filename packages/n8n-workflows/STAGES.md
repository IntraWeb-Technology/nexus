...
- The marketing site sends **`appointmentscheduled`** by default (valid on HubSpot’s **default** sales pipeline). For **custom** pipelines, set deployment env **`N8N_CONTACT_DEAL_STAGE`** on `iw-site` to that pipeline’s numeric stage id so new deals land in the correct column.
