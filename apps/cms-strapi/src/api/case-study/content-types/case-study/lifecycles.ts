...
const UID = 'api::case-study.case-study' as const;
...
/**
 * Best-effort slug uniqueness *per Site* for Case Study (many-to-many
...
export default {
  ...
  async beforeUpdate(event: { params: { where: Record<string, unknown>; data: Record<string, any> } }) {
    const { params } = event;
    const current: CaseStudyRow | null = params.where
...
async function assertUniqueSlugPerSite({
  ...
  const matches: CaseStudyRow[] = await strapi.db.query(UID).findMany({
    where: { slug },
    populate: { sites: true },
  });
  ...
  if (conflict) {
    throw new ApplicationError(`Another Case Study already uses slug "${slug}" on one of the selected Sites.`);
  }
}
