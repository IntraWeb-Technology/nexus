...
/**
 * Enforces "exactly one Home Page entry per Site" from
 * docs/strapi-migration/content-model.md. `site` is a one-to-one
 * relation stored as a plain FK column.
 *
 * Draft & Publish caveat: a Home Page document has separate draft
...
export default {
  ...
  async beforeUpdate(event: { params: { where: Record<string, unknown>; data: Record<string, any> } }) {
    ...
    const current: SingletonRow | null = params.where
      ? await strapi.db.query('api::home-page.home-page').findOne({ where: params.where })
      : null;
...
async function assertSinglePerSite({
  ...
  if (conflict) {
    throw new ApplicationError('This Site already has a Home Page entry. Only one is allowed per Site.');
  }
}
