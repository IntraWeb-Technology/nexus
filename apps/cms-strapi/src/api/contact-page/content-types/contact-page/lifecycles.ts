...
/**
 * Enforces "exactly one Contact Page entry per Site" from
 * docs/strapi-migration/content-model.md. `site` is a one-to-one
 * relation stored as a plain FK column.
 *
 * Draft & Publish caveat: a Contact Page document has separate draft
...
export default {
  ...
  async beforeUpdate(event: { params: { where: Record<string, unknown>; data: Record<string, any> } }) {
    ...
    const current: SingletonRow | null = params.where
      ? await strapi.db.query('api::contact-page.contact-page').findOne({ where: params.where })
      : null;
...
async function assertSinglePerSite({
  ...
  const matches: SingletonRow[] = await strapi.db
    .query('api::contact-page.contact-page')
    .findMany({ where: { site } });
  ...
  if (conflict) {
    throw new ApplicationError('This Site already has a Contact Page entry. Only one is allowed per Site.');
  }
}
