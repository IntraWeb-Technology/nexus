...
/**
 * Strapi publishing webhook → selective `revalidatePath`.
...
export async function POST(request: NextRequest) {
  ...
  if (!shouldRevalidateForSite(payload, INTRAWEB_SITE_KEY)) {
    return NextResponse.json({
      revalidated: false,
      paths: [],
      reason: `siteKeys does not include ${INTRAWEB_SITE_KEY}`,
    });
  }
