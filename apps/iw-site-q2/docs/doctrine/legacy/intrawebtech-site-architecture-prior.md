...
**Version:** 2.1 — Doctrine v2 reconciliation (review pass; IA not redesigned)  
**Authority:** Page structure and sequencing guidance. Does **not** redefine company identity. Positioning Foundation v2 governs identity. Supersedes blueprint Section 1 sequencing with full implementation-grade spec where still compatible.

> **SUPERSEDED — HISTORICAL REFERENCE ONLY**
>
> This document is not active site architecture.
> Site architecture authority resides in [`../intrawebtech-site-architecture.md`](../intrawebtech-site-architecture.md) (approved Site Architecture v2).
>
> Do not cite this file as governing homepage sequence, section responsibility, page purpose, or conversion architecture.
> Retained solely as historical reference for the pre-v2 architecture.

> **FOLLOW-UP — CONTENT ARCHITECTURE REVIEW REQUIRED (not performed in Doctrine v2 reconciliation)**
>
> Portions of this document still encode v1-era assumptions that need a later content-architecture pass before website implementation:
> - Hero category labels such as "Operational systems" / "Workflow infrastructure" as exclusive identity signals
> - Absolute prohibition on sentences beginning with "We build" above the fold (conflicts with v2 "we build" test)
> - Headcount-band fit language (20–150) treated as structural filter
> - Exclusive operational-infrastructure framing in page purpose statements
> - Capability / services taxonomy that may need remapping to Product Engineering, Systems Integration, Modernization & Rescue, Automation & AI, Production Reliability
>
> Do **not** treat those passages as license to redesign IA during doctrine-only work. Flag only.
...
Identity and buyer definition: use Positioning Foundation v2. If this architecture document conflicts with v2 on identity, buyer, or commercial boundaries, stop and ask — do not silently prefer this document.
...
- Generic capability theater (including empty "we build innovative solutions…" constructions). Meaningful "we build" engineering-responsibility language is evaluated under Positioning Foundation v2 — not categorically banned.
- Abstract nouns used as first-word identity: exclusive "operational infrastructure" as company category
- More than two sentences of body text
- Leading with AI adoption as company identity in the first visible viewport

> Content-architecture review flag: historical absolute ban on any sentence beginning with "We build" is superseded as doctrine; rewrite hero constraints in the later IA/content pass.
...
- Fit indicators: each one carries a consequence clause (not just the filter)
  - Headcount bands (e.g. historical "Teams of 20-150 people") are not root qualification under Positioning Foundation v2. Prefer engineering-condition language: "Teams where operational/production complexity has grown faster than the systems that support it."
  - Content-architecture review flag: remap fit copy in a later pass; do not redesign this section here.
