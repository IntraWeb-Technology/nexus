function Process() {
  const steps = [
    {
      n: '01', phase: 'DIAGNOSTIC', dur: '2–3 weeks', title: 'Map, identify, prioritize.',
      desc: 'We map your workflows, identify where automation makes sense, and build a prioritized roadmap. No commitment to ongoing work — if we\'re not the right fit, we\'ll tell you.',
      bullets: ['Workflow audit', 'ROI scoring per process', 'Prioritized roadmap', 'Go/no-go decision'],
    },
    {
      n: '02', phase: 'IMPLEMENTATION', dur: '6 mo minimum', title: 'Build, integrate, deploy.',
    ...
    {
      n: '03', phase: 'OPTIMIZATION', dur: 'ongoing', title: 'Monitor, tune, extend.',
  ...
  return (
    <section id="process" data-screen-label="04 Process" style={{ padding: '140px 0', position: 'relative' }}>
      <div className="container">
        ...
        <div style={{ marginTop: 72, display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 80 }}>
          {/* Left: stepper */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, position: 'relative' }}>
            {/* connector line */}
            <div style={{
              position: 'absolute', left: 24, top: 40, bottom: 40, width: 1,
              background: 'var(--iw-hairline)',
            }}/>
            <div style={{
              position: 'absolute', left: 24, top: 40, width: 1,
            ...
            {steps.map((s, i) => (
              <button key={i}
                ...
                <div style={{
                  ...
                  background: active === i ? 'var(--iw-void)' : 'var(--iw-void)',
                  borderRadius: '50%',
                  display: 'grid', placeItems: 'center',
                  fontFamily: 'var(--iw-mono)', fontSize: 11, fontWeight: 600,
                  color: active === i ? 'var(--accent)' : 'var(--iw-fg-2)',
                  transition: 'all 300ms',
                  boxShadow: active === i ? '0 0 0 6px rgba(52,231,208,0.1)' : 'none',
                }}>{s.n}</div>
                <div className="mono" style={{ fontSize: 10, color: 'var(--iw-fg-3)', letterSpacing: '0.2em', marginBottom: 6 }}>
                  {s.phase} · {s.dur}
                </div>
                <div style={{ fontFamily: 'var(--iw-display)', fontSize: 22, fontWeight: 600, letterSpacing: '-0.02em' }}>
                  {s.title}
                </div>
              </button>
            ))}
            ...
            <div style={{
              ...
              <div key={active} style={{ animation: 'reveal-up 500ms var(--ease-out)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                  ...
                  <span className="mono" style={{ fontSize: 11, color: 'var(--iw-fg-3)' }}>{steps[active].dur}</span>
                </div>

                <h3 style={{ fontSize: 36, fontWeight: 700, letterSpacing: '-0.03em', lineHeight: 1.05, marginBottom: 20 }}>
                  {steps[active].title}
                </h3>
