...
**Next milestone:** **M6 — Documentation** (after Editorial System Freeze Gate — **PASSED**)
...
| **M6 — Documentation** | **NEXT** |
| **M7 — Production Assets** | Planned |
| **M8 — Strapi Integration** | Planned |
| **M9 — Hardening** | Planned |
| **M10 — Launch** | Planned |

**Program posture:** Fixture-first until Documentation (M6) finishes proving handbook IA on the frozen publishing foundation. Strapi remains at **M8**. Do not reopen M0–M5 publishing architecture without a material defect. Do not begin M6 implementation from this documentation status alone — follow the M6 Manifest expectations when that milestone starts.
...
| **M6 — Documentation** | Handbook IA on proven publishing primitives | Freeze Gate passed | `/docs`, `/docs/[…slug]`, category-driven fixtures | Docs reuse PROVEN primitives; no second publishing system | **NEXT** |
...
**Result (2026-08-12): PASSED.** See [Editorial Publishing (M5)](/docs/architecture/editorial-publishing) for primitive classification (PROVEN / ARTICLE-LOCAL / DEFERRED), fixtures, and test evidence.
...
Program foundations through **M5** are complete; Editorial System Freeze Gate **PASSED**. Checklist below remains the launch readiness contract; items are not a blocker to begin **M6**.
...
| 10 | M0–M5 complete; Freeze Gate passed; M6 unambiguously next | YES |

### Open questions (non-blocking for M6; resolve before relevant later milestones)
...
**M0–M5 are complete.** Editorial System Freeze Gate **PASSED**. Frontend architecture v1.0 remains frozen for proven core routes; see [Architecture Freeze](/docs/architecture/architecture-freeze). Articles publishing report: [Editorial Publishing (M5)](/docs/architecture/editorial-publishing). Strapi remains deferred to **M8**; fixtures continue to document editorial field shapes. **M6 — Documentation is next.** Do not reopen M0–M5 publishing architecture without a material defect. Do not start Strapi or Launch side quests outside Manifest order.
...
### Validated (through M5 + Editorial System Freeze Gate)

- ✓ ChapterMarker — Home + Work + Case Study + Articles
- ✓ Figure (+ `tone`, optional `label` / `panels`) — Home, Work, Case Study, Articles
- ✓ MetadataRow — Case Study desktop metadata
- ✓ CaseStudyToc — Case Study + Articles D/T/M recomposition
- ✓ ReadingProgress — Case Study + Articles long-form chrome
- ✓ EditorialTable — Constraints / Delivery / Lessons (case-study matrix)
- ✓ EditorialDataTable — semantic comparison tables (Articles M5)
- ✓ EvidenceBlock — engineering evidence (Articles M5)
- ✓ Callout — tradeoff / note / warning (Articles M5)
- ✓ CodeBlock / TerminalBlock — Articles M5 (TS, YAML, terminal)
...
- ✓ Fixture-first Strapi-shaped content modules — including `article.ts` + `articles/*`
...
- Figure `src` / production evidence media (M7)
- Syntax highlighter / copy-to-clipboard chrome

---

**M0–M5 complete. Editorial System Freeze Gate PASSED. M6 (Documentation) is next.** Keep fixture-first until M8 Strapi. Do not start Strapi or Launch side quests outside Manifest order.
