...
**Design status:** Atlas Design v1.0 frozen (Editorial Brand System + page family); **Approval Polish (Page 27 / §29) implemented in frontend** — see [M9D Art Direction](/docs/architecture/m9d-art-direction)  
...
**Program posture:** M8 CMS contract implemented. **M9D** brings Approval Polish (Figma page 27 §29) into `atlas-web` chrome, Articles, and shared CTAs. Documentation Freeze **PASSED**. Production Asset Freeze **PASS**. Content source: Strapi when `STRAPI_URL` is set; explicit fixture mode for tests (`ATLAS_CONTENT_SOURCE=fixture`). **A-05 Vehicle Maintenance** media remains **deferred**. Live Strapi visual verification remains an external dependency for M9D.
...
| --- | --- |
| Visual identity & page composition | Figma pages 15–26 (frozen) + **Page 27 Approval Polish (§29)** for M9D chrome/Articles polish |
