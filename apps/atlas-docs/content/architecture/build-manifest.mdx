...
**Next milestone:** **M5 — Articles / Editorial Publishing** (do not expand beyond Manifest scope)
...
| **M5 — Articles / Editorial Publishing** | **NEXT** |
| Editorial System Freeze Gate | Pending (after M5; not a milestone) |
| **M6 — Documentation** | Planned |
| **M7 — Production Assets** | Planned |
| **M8 — Strapi Integration** | Planned |
| **M9 — Hardening** | Planned |
| **M10 — Launch** | Planned |

**Program posture:** Fixture-first until the editorial frontend contract is proven through Articles (M5) and Documentation (M6). Strapi remains at **M8**. Do not reopen M0–M4. Do not begin M5 implementation from this documentation pass.

### Milestone scheme note

Earlier implementation reports used an implementation-local numbering (**M0 Foundations → M1 Home → M2 Work → M3 Case Study**). Those reports remain valid historical proof (Homepage Pilot, Work Route, Case Study Route, Architecture Freeze). As of 2026-08-11 this Build Manifest uses the **program-level** milestone scheme above. That earlier core-route implementation work is completed under **M4 — Editorial Brand / Core Route Implementation**.
...
### Folder structure (actual — through M4 core routes)
...
│   └── sections/               # page-local compositions (home-*, work-*, case-*)
├── content/
...
└── e2e/                        # Playwright (homepage, work, case-study)
```
...
| Docs landing / docs | **M6** on proven primitives; CMS at M8 |
...
| `/articles` | Editorial writing index | Site chrome | Featured writing, chronological writing, editorial metadata, restrained topic nav (Figma authority) | Editorial hierarchy — **not** a card-grid blog | Fixture (M5) | Collection | `CollectionPage` | Journey + visual | **M5 NEXT** |
| `/articles/[slug]` | Article / publishing proving ground | Site chrome + TOC/progress as designed | Header, title/deck, metadata, prose, figures, code, callouts, evidence, tables, related, prev/next | Page 26 primitives | Fixture (M5) | Article | `Article` / `TechArticle` | Journey + a11y + visual | **M5 NEXT** |
| `/about` | Editorial profile | Site chrome | Opening, Philosophy, Approach, Style, Principles, Timeline, Focus, Reading, Architecture, Notes, Contact | Pull quote, timeline, diagrams | Partial | Profile | `ProfilePage` / `Person` | Journey + visual | Design frozen (not next; do not reopen M4) |
| `/docs` | Handbook landing | Site chrome | Header, Search UI, Categories, Recently Updated, Progress note | Search field, category cards, list rows | Fixture → CMS at M8 | Collection | `CollectionPage` | Journey | **M6** (after Editorial System Freeze) |
| `/docs/[…slug]` | Doc article | Site chrome + progress | Article patterns from Page 26 | Proven editorial primitives from M5 gate | Fixture → CMS at M8 | Article | `TechArticle` | Journey + visual | **M6** |
| `/contact` | Conversation starter | Site chrome | Quiet hero + 3-field form | Form controls | Form only | Contact | — | Journey + form | Design frozen (not next; do not reopen M4) |
...
- **Articles (`/articles`, `/articles/[slug]`):** **M5** is the first production proving ground for the complete Atlas Editorial Publishing System (Figma Page 26). Not a deferred nice-to-have.
- **About / Contact:** Designs are frozen in Figma (pages 20 / 23). Nav already targets these routes. They are **not** the next implementation milestone; do not describe them as “M4 to begin.” Remaining page implementation stays in launch scope without renumbering M5–M10.
- **Search on `/docs`:** UI pattern approved; backend may be static filter or deferred pagefind — decide during M6 without inventing AI search.
...
3. **`article`** — Docs categories via `category`; use for engineering notes / ADRs / playbooks. Avoid inventing five collections.
...
| **M4 — Editorial Brand / Core Route Implementation** | Brand system + core frontend routes | M0–M3, design freeze | Editorial Brand System in Figma; `/`, `/work`, `/work/[slug]` implemented fixture-first | Matches frozen brand + pages 15/18/21; architecture freeze | **COMPLETE** — do not restart |
| **M5 — Articles / Editorial Publishing** | Prove Editorial Publishing System in production UI | M4 complete | `/articles`, `/articles/[slug]`, representative fixtures, reusable primitives | See **M5 expectations** below + Editorial System Freeze Gate | **NEXT** |
| *Editorial System Freeze Gate* | Acceptance gate only (not a milestone) | M5 implementation | Primitive classification + visual/a11y/responsive sign-off | See **Editorial System Freeze Gate** below | Pending after M5 |
| **M6 — Documentation** | Docs experience on proven editorial system | Freeze gate passed | `/docs`, doc articles using **PROVEN** primitives | Matches page 22 + Page 26 proven set | Planned |
| **M7 — Production Assets** | Formal production visual/evidence asset pass | M5–M6 opportunistic capture underway | Real assets replace fixtures/placeholders | Asset manifest (§7) complete for launch set | Planned |
| **M8 — Strapi Integration** | Map proven frontend contract → shared CMS | M5–M6 contract proven | `personal` site content via `cms-strapi` + `@repo/strapi-client` | Site isolation; no speculative model drift | Planned |
...
M5 is **not** merely scaffolding `/articles` and `/articles/[slug]`. It is the first production proving ground for the complete **Atlas Editorial Publishing System** (Figma Page 26).
...
```text
M5 Articles  →  Editorial System Freeze Gate  →  M6 Documentation
```
...
| --- | --- | --- | --- |
| Missing real evidence assets delay launch | **H** | **H** | **Open — high priority.** Asset manifest owners; labeled placeholders; **opportunistic capture in M5/M6**; formal completion in **M7** |
...
| Contact spam | M | M | CAPTCHA/honeypot + rate limit when Contact ships |
...
Program foundations through M4 are complete. Checklist below remains the launch readiness contract; items are not a blocker to begin **M5**.
...
| 4 | Assets tracked (§7) | YES (sheet to be owned; opportunistic capture from M5) |
| 5 | CMS mapped (§6) | YES (plan only — **fixture-first through M6; integrate at M8**) |
| 6 | Accessibility planned (§9) | YES |
| 7 | SEO planned (§10) | YES |
| 8 | Testing planned (§11) | YES |
| 9 | CI/CD understood (§12) | YES |
| 10 | M0–M4 complete; M5 unambiguously next | YES |

### Open questions (non-blocking for M5; resolve before relevant later milestones)

1. Extend existing `case-study` / `article` vs Atlas-only types? — resolve before **M8** schema merge.  
2. Docs search: client filter vs pagefind vs defer? — resolve during **M6**.  
3. Contact delivery: Resend vs existing Nexus form path? — resolve when Contact implementation is scheduled.  
4. Legacy URL inventory completeness for redirects? — resolve before **M9/M10** cutover.  
5. About / Contact implementation slot within M6–M10 without renumbering? — decide without reopening M4 or delaying M5.
...
- Full docs article route — **M6** after Editorial System Freeze Gate.
...
**M0–M4 are complete.** Frontend architecture v1.0 remains frozen for proven core routes; see [Architecture Freeze](/docs/architecture/architecture-freeze) (historical M0–M3 implementation labels). Strapi remains deferred to **M8**; fixtures continue to document editorial field shapes. **M5 — Articles / Editorial Publishing is next.** Do not reopen M0–M4. Do not start Documentation, Strapi, or Launch side quests outside Manifest order.
...
### Validated (through M4 core routes)
...
- ✗ ContactBridge — Home/Work/Case compositions differ; extract when a true shared shell exists
...
### Deferred (to prove in M5 / classify at Freeze Gate)

Page 26 publishing primitives still to be exercised in Articles (then classified PROVEN / ARTICLE-LOCAL / DEFERRED):
...
- Articles index editorial hierarchy (non-card-grid)

---

**M0–M4 complete. M5 (Articles / Editorial Publishing) is next.** Pass the Editorial System Freeze Gate before M6 Documentation. Keep fixture-first until M8 Strapi. Do not start Docs, Strapi, or additional scope as side quests outside Manifest order.
