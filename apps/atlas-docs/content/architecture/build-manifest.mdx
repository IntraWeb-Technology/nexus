...
**Status:** Approved — implementation contract (Milestone 5)  
...
**Work route:** Implemented — see [Work Route](/docs/architecture/work-route). Additional routes await architectural approval.
...
- Ship core journeys: Home, Work, Case Study, About, Contact, Documentation landing.
...
│   │   ├── docs/
│   │   └── not-found.tsx
│   ├── api/                    # contact, preview, revalidate only as needed
│   ├── layout.tsx
│   ├── robots.ts
│   └── sitemap.ts
├── components/
│   ├── chrome/                 # Nav, Footer, ReadingProgress
│   ├── editorial/              # Figure, Callout, CodeBlock, Table, …
│   ├── sections/               # page-section compositions from Figma
│   └── ui/                     # tiny primitives (Button link, Input) if needed
├── lib/
│   ├── strapi/                 # mappers → app types
│   ├── seo/
│   └── utils/
├── styles/
...
│   └── globals.css
├── types/
└── tests/                      # unit + Playwright colocated or e2e/
public/
└── images/                     # static fallbacks only; prefer CMS media
```
...
| Docs landing | Hybrid: static category IA + CMS “recent” articles/notes |
...
| `/work/[slug]` | Case study | Site chrome + TOC/progress | Header, TOC, Hero, Overview… Contact (template 21) | Article header, TOC, figures, diagrams, evidence, related | Yes | Article | `Article` / `TechArticle` | Journey + visual | Design frozen |
| `/about` | Editorial profile | Site chrome | Opening, Philosophy, Approach, Style, Principles, Timeline, Focus, Reading, Architecture, Notes, Contact | Pull quote, timeline, diagrams | Partial | Profile | `ProfilePage` / `Person` | Journey + visual | Design frozen |
| `/docs` | Handbook landing | Site chrome | Header, Search UI, Categories, Recently Updated, Progress note | Search field, category cards, list rows | Partial | Collection | `CollectionPage` | Journey | Design frozen |
| `/docs/[…slug]` | Doc article (v1.1 if needed) | Site chrome + progress | Article patterns from 26 | Editorial primitives | Yes | Article | `TechArticle` | When in scope | Deferred unless launch requires |
| `/contact` | Conversation starter | Site chrome | Quiet hero + 3-field form | Form controls | Form only | Contact | — | Journey + form | Design frozen |
...
- **Articles:** only if launch scope requires; otherwise Docs landing + case studies suffice.
...
| Playwright | Home, Work, Case, About, Contact, Docs, 404, nav, mobile menu | P0 |
...
| **M3 — Case study** | Template 21 + CMS fields | M2, schema extension | `/work/[slug]` | Matches template; evidence figures | L |
| **M4 — About + Contact** | Profile + form | M0 | `/about`, `/contact` | Matches 20/23; form delivers | M |
| **M5 — Docs landing** | `/docs` | M0, articles/categories | Docs landing | Matches 22 | S/M |
| **M6 — SEO + redirects** | Metadata, sitemap, legacy URLs | Content routes | SEO plumbing | Sitemap valid; redirects | S |
| **M7 — Hardening** | Tests, a11y, visuals, perf, security headers | M1–M6 | CI green | §11–§13 | M |
...
3. Contact delivery: Resend vs existing Nexus form path?  
...
flowchart TD
  ...
  M0 --> M5[M5 Docs landing]
  M1 --> M6[M6 SEO + redirects]
  M3 --> M6
  M4 --> M6
  M5 --> M6
  M6 --> M7[M7 Hardening]
...
- Full docs article template route — deferred unless launch requires.
...
This manifest is sufficient for **M0–M2** (Home + Work implemented fixture-first). **M3** requires a short ADR choosing case-study schema strategy, then implementation may proceed without inventing presentation outside Pages 15–26.

---

## Proven Patterns

Validated only after Home + Work implementation succeeded. Do not mark unimplemented patterns as validated.

### Validated

- ✓ ChapterMarker — Home + Work
- ✓ Figure (+ `tone`) — Home featured + Work featured
- ✓ Navigation (`SiteNav` + route-aware active) — chrome
- ✓ Footer (`SiteFooter`) — chrome
- ✓ Page-local section compositions — no section registry
- ✓ Fixture-first Strapi-shaped content modules — `homepage.ts`, `work.ts`
- ✓ RSC default · client island only for nav active
- ✓ Playwright D/T/M + axe + visual baselines

### Intentionally not extracted (yet)

- ✗ ContactBridge — Home/Work compositions differ; extract when a true shared shell exists
- ✗ MetadataRow — Work-only pattern so far
...
- Caption as standalone primitive (lives on `Figure` today)
- ReadingProgress / CaseStudyToc — Case Study milestone
- PullQuote / Callout / EvidenceBlock — Case Study / About

---

**Stop.** Do not begin Case Study implementation until Work route architectural review completes and the M3 schema ADR is decided.
