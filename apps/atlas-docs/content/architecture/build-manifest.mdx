...
**Next milestone:** **M7 — Production Assets** (after Documentation Freeze — **PASSED**)
...
| **M7 — Production Assets** | **NEXT** |
| **M8 — Strapi Integration** | Planned |
| **M9 — Hardening** | Planned |
| **M10 — Launch** | Planned |

**Program posture:** Fixture-first until Strapi (**M8**). Documentation Freeze **PASSED** — M7 may replace production assets/copy without Documentation architecture changes. Do not reopen M0–M6 without a material defect.
...
| **M7 — Production Assets** | Formal production visual/evidence asset pass | M5–M6 opportunistic capture underway | Real assets replace fixtures/placeholders | Asset manifest (§7) complete for launch set | **NEXT** |
...
**M0–M6 are complete.** Editorial System Freeze Gate **PASSED**. Documentation Freeze **PASSED**. Frontend architecture v1.0 remains frozen for proven core routes; see [Architecture Freeze](/docs/architecture/architecture-freeze). Articles publishing report: [Editorial Publishing (M5)](/docs/architecture/editorial-publishing). Documentation report: [Documentation System (M6)](/docs/architecture/documentation-system). Strapi remains deferred to **M8**; fixtures continue to document editorial field shapes. **M7 — Production Assets is next.** Do not reopen M0–M6 without a material defect. Do not start Strapi or Launch side quests outside Manifest order.
...
### Validated (through M6 + Documentation Freeze)

- ✓ ChapterMarker — Home + Work + Case Study + Articles + Docs
- ✓ Figure (+ `tone`, optional `label` / `panels`) — Home, Work, Case Study, Articles, Docs
...
**M0–M6 complete. Editorial System Freeze Gate PASSED. Documentation Freeze PASSED. M7 (Production Assets) is next.** Keep fixture-first until M8 Strapi. Do not start Strapi or Launch side quests outside Manifest order.
