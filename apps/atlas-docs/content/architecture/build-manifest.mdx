...
**Next milestone:** **M8 — Strapi Integration** (after Production Asset Freeze — **BLOCKED** on A-05)
...
| **M7 — Production Assets** | **ACTIVE** — Freeze **BLOCKED** (A-05) |
| **M8 — Strapi Integration** | **PLANNED** |
| **M9 — Hardening** | Planned |
| **M10 — Launch** | Planned |

**Program posture:** Fixture-first until Strapi (**M8**). Documentation Freeze **PASSED**. Production assets largely integrated; **A-05 Vehicle Maintenance** remains blocked without a production source. Do not reopen M0–M6 without a material defect. Do not start Strapi until Freeze clears or A-05 is removed from launch scope by authority.
...
| A-05 | Selected work — Vehicle Maintenance | Eng | P0 | Per card | Named project | Yes | Production | **BLOCKED** |
...
**Production Asset Freeze:** **BLOCKED** — A-05 lacks a production source while remaining launch selected work. Report: [Production Assets (M7)](/docs/architecture/production-assets).
...
| **M7 — Production Assets** | Formal production visual/evidence asset pass | M5–M6 opportunistic capture underway | Real assets replace fixtures/placeholders | Asset manifest (§7) complete for launch set | **ACTIVE** — Freeze **BLOCKED** (A-05) |
...
```text
M5 Articles  →  Editorial System Freeze Gate PASSED  →  M6 Documentation COMPLETE  →  Documentation Freeze PASSED  →  M7 NEXT
```
...
| --- | --- | --- | --- |
| Missing real evidence assets delay launch | **H** | **H** | **Open — A-05 Vehicle Maintenance blocked.** Other P0 assets production/composed; see [Production Assets (M7)](/docs/architecture/production-assets) |
...
Program foundations through **M6** are complete; Editorial System Freeze Gate **PASSED**; Documentation Freeze **PASSED**. Checklist below remains the launch readiness contract; items are not a blocker to begin **M7**.
...
| 4 | Assets tracked (§7) | YES (sheet to be owned; opportunistic capture from M5–M6) |
...
| 10 | M0–M6 complete; Freeze Gates passed; M7 unambiguously next | YES |

### Open questions (non-blocking for M7; resolve before relevant later milestones)
...
**M0–M6 are complete.** Editorial System Freeze Gate **PASSED**. Documentation Freeze **PASSED**. Frontend architecture v1.0 remains frozen for proven core routes; see [Architecture Freeze](/docs/architecture/architecture-freeze). Articles publishing report: [Editorial Publishing (M5)](/docs/architecture/editorial-publishing). Documentation report: [Documentation System (M6)](/docs/architecture/documentation-system). Production assets report: [Production Assets (M7)](/docs/architecture/production-assets). **M7 is ACTIVE** with Production Asset Freeze **BLOCKED** on **A-05**. Strapi remains deferred to **M8**; fixtures continue to document editorial field shapes. Do not reopen M0–M6 without a material defect. Do not start Strapi or Launch side quests outside Manifest order.
...
- A-05 Vehicle Maintenance production media (blocked — source required)
...
**M0–M6 complete. Editorial System Freeze Gate PASSED. Documentation Freeze PASSED. M7 ACTIVE — Production Asset Freeze BLOCKED (A-05).** Keep fixture-first until M8 Strapi. Do not start Strapi or Launch side quests outside Manifest order.
