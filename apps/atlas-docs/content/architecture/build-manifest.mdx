...
**Program posture:** M8 implemented the Strapi schemas, client contract, content assemblers, and integration code in `apps/atlas-web/src/lib/strapi/` and `@repo/strapi-client`. **Authentic frontend-visible CMS delivery remains unverified** until a reachable Strapi instance is populated and traced through Atlas. **M9D** brings Approval Polish into `atlas-web` chrome, Articles, and shared CTAs. Content source: Strapi when `STRAPI_URL` is set (fail-closed — no silent fixture fallback); explicit fixture mode for local development, Playwright, and demos (`ATLAS_CONTENT_SOURCE=fixture`). **A-05 Vehicle Maintenance** media remains **deferred**.
