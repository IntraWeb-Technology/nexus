...
**Next milestone:** **M8B — Strapi Implementation** (after M8A Content Contract — **LOCKED**; A-05 deferred)
...
| **M8 — Strapi Integration** | **M8A COMPLETE** — Content Contract **LOCKED**; **M8B PLANNED** |
| **M9 — Hardening** | Planned |
| **M10 — Launch** | Planned |

**Program posture:** Fixture-first until Strapi implementation (**M8B**). Documentation Freeze **PASSED**. Production Asset Freeze **PASS**. **M8A Content Contract LOCKED** — see [M8 Content Contract](/docs/architecture/m8-content-contract). **A-05 Vehicle Maintenance** media is consciously **deferred** (no approved production source; substitute evidence forbidden). Do not reopen M0–M7 without a material defect. Do not invent CMS architecture during M8B — execute the locked contract.
...
| **M8 — Strapi Integration** | Map proven frontend contract → shared CMS | M7 Freeze PASS + M8A contract lock | `personal` site content via `cms-strapi` + `@repo/strapi-client` | Site isolation; no speculative model drift | **M8A COMPLETE** (contract locked) · **M8B PLANNED** |
...
```text
M5 Articles  →  Editorial System Freeze Gate PASSED  →  M6 Documentation COMPLETE  →  Documentation Freeze PASSED  →  M7 COMPLETE  →  Production Asset Freeze PASS  →  M8A Content Contract LOCKED  →  M8B NEXT
```
...
Program foundations through **M7** are complete; Editorial System Freeze Gate **PASSED**; Documentation Freeze **PASSED**; Production Asset Freeze **PASS**; M8A Content Contract **LOCKED**. Checklist below remains the launch readiness contract; items are not a blocker to begin **M8B**.
...
| 10 | M0–M7 complete; Freeze Gates passed; M8A locked; M8B unambiguously next | YES |
