...
function toProjectCard(projectMeta: ProjectMeta | Record<string, unknown>): Project {
  const meta = projectMeta as ProjectMeta;
  return {
    id: meta.id,
    title: meta.title,
    description: meta.description,
    image: meta.image || '/assets/hero/hero-image.webp',
    tags: meta.tags || [],
...
function ProjectsPageClient({
  ...
  const projects: Project[] = useMemo(
    () => (initialProjects?.length ? initialProjects : projectMetaList).map(toProjectCard),
    [initialProjects],
  );
...
export default function ProjectsPageClientWrapper({
  initialProjects,
}: {
  initialProjects?: ProjectMeta[];
}) {
